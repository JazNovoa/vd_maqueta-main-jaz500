<script>
    ...
    // Script para desplazar horizontalmente con la rueda del ratón
    function handleWheel(event) {
        if (event.deltaY !== 0) {
            event.preventDefault(); // Previene el desplazamiento vertical
            let scrollSpeed = 20; // Ajusta este valor para aumentar la velocidad de desplazamiento
            document.querySelector('.scrollable-content').scrollLeft += event.deltaY * scrollSpeed; // Desplaza horizontalmente más rápido
        }
    }
